...
11. Never expose the manager sidecar port publicly. If remote/mobile UI is needed, use only Viewer Board on its own port or a Gateway/reverse-proxy URL, and never reuse the public ingest host:port as a UI origin.
